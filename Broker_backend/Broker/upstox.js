...
const place_order = async (
  ...
  try {
    ...
    if (token != 0) {
      ...
      const command = `findstr /C:"${pattern}" ${filePath_token}`;
              ...
              axios(config)
                .then(async (response) => {
                  if (response.data.data != undefined) {
                    ...
                    const Exist_entry_order = response.data.data.find(
                      (item1) => item1.instrument_token == parts
                    );

                    if (Exist_entry_order != undefined) {
                      item.postdata.instrument_token = parts;
                      const possition_qty =
                        Exist_entry_order.day_buy_quantity -
                        Exist_entry_order.day_sell_quantity;
                      ...
                      if (possition_qty == 0) {
                        BrokerResponse.create({
                          ...
                          .catch((err) => {
                            try {
                              console.log(
                                "Error creating and saving user:",
                                err
                              );
                            } catch (e) {}
                          });
                ...
                .catch(async (error) => {
                  fs.appendFile(
                    ...
                    "TIME " +
                      new Date() +
                      " UPSTOX POSITION DATA ERROR CATCH - " +
                      item.UserName +
                      " ERROR - " +
...
const EntryPlaceOrder = async (item, filePath, signals, signal_req) => {
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " UPSTOX BEFORE PLACE ORDER USER ENTRY- " +
      item.UserName +
      " REQUEST -" +
  ...
  axios(config)
    .then(async (response) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " UPSTOX AFTER PLACE ORDER USER ENTRY - " +
          item.UserName +
          " RESPONSE -" +
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " UPSTOX AFTER PLACE ORDER CATCH ENTRY - " +
          item.UserName +
          " ERROR -" +
          JSON.stringify(error) +
          "\n",
...
const ExitPlaceOrder = async (
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " UPSTOX BEFORE PLACE ORDER USER EXIT- " +
      item.UserName +
      " REQUEST -" +
      JSON.stringify(item.postdata) +
      "\n",
  ...
  axios(config)
    .then(async (response) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " UPSTOX AFTER PLACE ORDER USER EXIT- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(response.data) +
          "\n",
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " UPSTOX AFTER PLACE ORDER USER EXIT CATCH- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(error) +
          "\n",
