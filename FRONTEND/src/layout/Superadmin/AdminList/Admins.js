...
const AdminsList = () => {
  ...
  const columns = [
    ...
    {
      dataFeild: "UpdateSericeToken",
      text: "Update Service Token",
      formatter: (cell, row) => (
        <span style={{ display: "flex" }}>
          <div className="tooltip-wrapper" title="Update Service Token">
            <RefreshCcw
              size={20}
              color="#198754"
              strokeWidth={2}
              className="mx-1 pointer-icon"
              onClick={() => UpdateServiceToken(row)}
            />
          </div>
        </span>
      ),
  ...
  const UpdateServiceToken = async (row) => {
    try {
      const { value: password } = await Swal.fire({
        title: "Enter your Server password",
        input: "text",
      ...
      let config = {
        method: "get",
        maxBodyLength: Infinity,
        url: row.domain + "/backend/services/update",
        headers: {
          "Content-Type": "application/json",
        },
        data: req,
      ...
      if (response.data.status) {
        Swal.fire({
          icon: "success",
          title: "Success",
          text: "Serive Token update successful.",
        });
      } else {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Service Token update failed.",
        });
      }
    } catch (error) {
      Swal.fire({
